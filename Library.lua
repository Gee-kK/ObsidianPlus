	...
	function Window:AddUniqueTab(...)
		...
		local function AddTabbox(self, Info)
			...
			local BoxHolder = New("Frame", {
				...
				BackgroundTransparency = 1,
				Size = UDim2.fromScale(1, 0),
				Parent = if ParentObj.Type == "Groupbox" then ParentObj.Container else (Info.Side == 1 and TabLeft),
			})
			New("UIListLayout", {
				Padding = UDim.new(0, 6),
				Parent = BoxHolder,
		...
		function Tab:AddGroupbox(Info)
			local BoxHolder = New("Frame", {
				...
				BackgroundTransparency = 1,
				Size = UDim2.fromScale(1, 0),
				Parent = Info.Side == 1 and TabLeft,
			})
			New("UIListLayout", {
				Padding = UDim.new(0, 6),
				Parent = BoxHolder,
...
function Library:CreateKeyWindow(WindowInfo)
	WindowInfo = Library:Validate(WindowInfo, Templates.KeyWindow)
	local ViewportSize: Vector2 = workspace.CurrentCamera.ViewportSize
	if RunService:IsStudio() and ViewportSize.X <= 5 and ViewportSize.Y <= 5 then
		repeat
			ViewportSize = workspace.CurrentCamera.ViewportSize
